<?php
    ...
    class Gestormodel extends CI_model{
        ...
        public function listaUsuario(){
            ...
            $this->db->join('nivelacesso n','u.id_nivel = n.idnivel','inner');
        ...
        // Função que faz a busca pro relatório
        public function relProdutos(){
            $periodo = date('Y-m-d',strtotime(str_replace('/','-',$this->input->post('periodo'))));
            $ate = date('Y-m-d',strtotime(str_replace('/','-',$this->input->post('ate'))));

            $this->db->select("*");
            $this->db->from("V_RelEstoque");
            $this->db->where("dtcadastro >= '$periodo' AND dtcadastro <= '$ate' ");
            $this->db->order_by("nomeProduto ASC");
        ...
        // SQL DE RELATÓRIO DE CLIENTES
        public function relCliente(){
            $periodo = date('Y-m-d', strtotime(str_replace('/','-',$this->input->post('periodo'))));
            $ate     = date('Y-m-d', strtotime(str_replace('/','-',$this->input->post('ate'))));

            $this->db->select("*");
            $this->db->from("V_RelClientes");
            $this->db->where("dtcadastro >= '$periodo' AND dtcadastro <= '$ate' ");
        ...
        // SQL DE RELATÓRIO DE VENDAS
        public function relVendas(){
            // pega os valores do campo input
            $periodo = date('Y-d-m',strtotime(str_replace('/','-',$this->input->post('periodo'))));
            $ate = date('Y-d-m',strtotime(str_replace('/','-',$this->input->post('ate'))));

            $this->db->select("*");
            $this->db->from("V_RelVendas");
            $this->db->where("dtcadastro >= '$periodo' AND dtcadastro <= '$ate' ");
            $this->db->order_by("nomeCliente ASC");
